...
export default function MySharedReadsList() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const response = await fetch(BASE_URL + "/backend/files/shared/reads/" + session.userID, {
                    headers: {
                ...
                setFiles(data.filter((file) => file.file_type === "reads"));
            } catch (error) {
                ...
                <main>
                    ...
                    <div className="bg-slate-100 px-5 text-sm">
                        <div className="bg-white rounded-md px-7 py-5 space-y-3">
                            <p className="font-bold text-md mb-3">Shared reads</p>
                            {files.length > 0 ? (
                                files.map((file) => {
                                    console.log(file)
                                        return (
                                ...
                                <p className="text-center p-5">No reads files shared to you.</p>
                            )}
                        </div>
                    </div>
                </main>
